...
<div style="margin:10px;padding:10px;">
	...
	<div>
		...
		<span name="${folder.id}" class="greybutton buttonicon deleteprogram">Delete program</span>
		</g:if>
		...
		<span name="${folder.id}" class="greybutton buttonicon addfolder">Add new folder</span>
		<span name="${folder.id}" class="greybutton buttonicon deletestudy">Delete study</span>
		</g:if>
	...
	<div class="description">
		...
		<g:else>${bioDataObject.description}</g:else>
	</div>
	...
	<div class="description">
		...
		<g:else>${bioDataObject.longDescription}</g:else>
	</div>
	<div style="height:20px;"></div>
	</g:elseif>

	<g:if test="${subjectLevelDataAvailable}">
	<center><div class="messagebox">Subject-level data is available for this study.
		<a href="#" onclick="openInAnalyze('${bioDataObject.accession}', '${createLink(controller: 'datasetExplorer')}');">Open in Analyze view</a>
	</div>
	</center>
	<br/><br/>
	</g:if>
		...
		<g:each in="${metaDataTagItems}" status='i' var='amTagItem'>
			<g:if test="${amTagItem.viewInGrid}">
				<tr class='details-row ${(i % 2) == 0 ? 'odd' : 'even'}'>
					...
					<!-- FIXED -->
					<g:if test="${amTagItem.tagItemType == 'FIXED' && amTagItem.tagItemAttr ? bioDataObject?.hasProperty(amTagItem.tagItemAttr) : false}">
	...
	<g:if test="${folder?.hasProperty('fmFiles') && folder.fmFiles}">
		<div class="dataTables_wrapper">
			...
			<div id="files-table">
				<g:render template='filesTable' model="[folder: folder]"/>
			</div>
		</div>
	</g:if>
	<span></span>
...
<%-- If this is an analysis, provide a div for analysis data --%>
	<g:if test="${folder.folderType.equalsIgnoreCase(FolderType.ANALYSIS.name())}">
		<center><div id="partialanalysiswarning" class="messagebox" style="display: none;">
			...
			<a id="loadfullanalysis" href="#" onclick="updateAnalysisData(${folder.id}, true)">Load all X</a></div>
		</center>
